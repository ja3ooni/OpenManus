...
class PerformanceTestRunner:
    ...
    async def run_test(
        self, test_name: str, test_function: Optional[Callable] = None
    ...
    async def _run_warmup(
        self, config: PerformanceTestConfig, test_function: Optional[Callable]
    ):
    ...
    async def _test_worker(
        ...
        request_interval = (
            config.concurrent_users / config.target_rps
    ...
    def _calculate_test_metrics(
        self, result: PerformanceTestResult, config: PerformanceTestConfig
    ):
        ...
        result.meets_targets = (
            ...
            and result.requests_per_second >= config.target_rps * 0.8  # 80% of target
            and result.error_rate_percent <= config.max_error_rate_percent
        )
    ...
    def _assess_performance_level(
        ...
        throughput_ratio = (
            result.requests_per_second / config.target_rps
            if config.target_rps > 0
            else 1.0
        )
    ...
    def _create_skipped_result(
        self, test_name: str, config: PerformanceTestConfig
    ) -> PerformanceTestResult:
    ...
    async def run_regression_test(
        ...
        response_time_change = self._calculate_percentage_change(
            baseline_result.avg_response_time_ms, current_result.avg_response_time_ms
        )
        ...
        error_rate_change = self._calculate_percentage_change(
            baseline_result.error_rate_percent, current_result.error_rate_percent
        )
        ...
        performance_change = (
            ...
            + -throughput_change
            * 0.4  # Throughput weight (negative because higher is better)
            + error_rate_change * 0.2  # Error rate weight
        )

        # Detect regression
        regression_detected = (
            abs(performance_change) > regression_threshold_percent
            or response_time_change > regression_threshold_percent
            or throughput_change < -regression_threshold_percent
            or error_rate_change > regression_threshold_percent
        )
    ...
    def get_test_history(
        self, test_name: str, limit: int = 10
    ) -> List[PerformanceTestResult]:
        """Get test execution history"""
        test_results = [
            result for result in self.test_results if result.test_name == test_name
        ]
    ...
    def get_test_summary(self) -> Dict[str, Any]:
        ...
        for test_name, results in tests_by_name.items():
            ...
            summary["test_summaries"][test_name] = {
                ...
                "success_rate": len(
                    [r for r in results if r.status == TestStatus.COMPLETED]
                )
                / len(results),
                "avg_response_time_ms": (
                    ...
                    if any(r.avg_response_time_ms > 0 for r in results)
                    else 0.0
                ),
                "avg_throughput_rps": (
                    statistics.mean(
                    ...
                    if any(r.requests_per_second > 0 for r in results)
                    else 0.0
                ),
            }
...
async def run_performance_test(
    test_name: str, test_function: Optional[Callable] = None
) -> PerformanceTestResult:
    """Run a specific performance test"""
...
async def run_all_performance_tests(
    test_function: Optional[Callable] = None, test_filter: Optional[List[str]] = None
) -> List[PerformanceTestResult]:
    """Run all configured performance tests"""
